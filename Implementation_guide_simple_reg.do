...
local omega_dist normal
	...
	levelsof cell, local(cells)  //save list of cells in local
		...
		replace theta_g = _b[parent_rank] if cell == `g'
		replace SE_theta_g = _se[parent_rank] if cell == `g'
			...
			forvalues i = 0/3 {
				...
				qui reg kid_rank parent_rank if cell==`g'
				replace theta_g_d = _b[parent_rank] if cell == `g'
	...
	* ! Repeat steps 1 - 3 treating SE_noise_infused_theta_g as the relevant parameter (i.e. theta)
	...
	export excel using "example_cell_public_estimates.xlsx", replace firstr(var)
